...
int openxr_init(void)
{
    if (g_openxr_state.initialized) {
        SPDLOG_WARN("OpenXR already initialized");
    ...
    SPDLOG_WARN("Initializing OpenXR context...");
    ...
    if (g_openxr_state.xrSystemId == XR_NULL_SYSTEM_ID) {
        SPDLOG_WARN("Failed to get OpenXR system. VR will not be available.");
    ...
    if (instanceExtensions.empty()) {
        SPDLOG_WARN("Failed to get Vulkan instance requirements. VR will not be available.");
    ...
    if (g_openxr_state.vkInstance == VK_NULL_HANDLE) {
        SPDLOG_WARN("Failed to create Vulkan instance. VR will not be available.");
    ...
    if (g_openxr_state.vkPhysicalDevice == VK_NULL_HANDLE) {
        SPDLOG_WARN("Failed to get Vulkan physical device. VR will not be available.");
    ...
    if (g_openxr_state.queueFamilyIndex < 0) {
        SPDLOG_WARN("Failed to get Vulkan queue family. VR will not be available.");
    ...
    if (g_openxr_state.vkDevice == VK_NULL_HANDLE) {
        SPDLOG_WARN("Failed to create Vulkan device. VR will not be available.");
    ...
    if (g_openxr_state.xrSession == XR_NULL_HANDLE) {
        SPDLOG_WARN("Failed to create OpenXR session. VR will not be available.");
    ...
    if (g_openxr_state.xrSpace == XR_NULL_HANDLE) {
        SPDLOG_WARN("Failed to create OpenXR reference space. VR will not be available.");
    ...
    if (!vr_renderer_init()) {
        SPDLOG_WARN("Warning: Failed to initialize VR renderer. VR rendering will not be available.");
