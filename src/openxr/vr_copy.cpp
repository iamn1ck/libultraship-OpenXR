...
#endif
// maybe need the linux for android?? 
...
int vr_copy_framebuffer_to_swapchain(int eye)
{
    ...
    if (!g_vr_copy.initialized || eye < 0 || eye > 1) {
        if (!logged_once) {
            SPDLOG_WARN("vr_copy not initialized or invalid eye index");
    ...
    if (!eyeState->initialized || !eyeState->stagingMapped) {
        if (!logged_once) {
            SPDLOG_WARN("vr_copy state not initialized or staging not mapped");
    ...
    if (sourceFBO == 0) {
            SPDLOG_WARN("Failed to get source framebuffer for eye d");
    ...
    if (frame_count < 5) {
            SPDLOG_WARN("DEBUG vr_copy: Frame{}", frame_count);
    ...
    if (swapchainImage == VK_NULL_HANDLE) {
            SPDLOG_WARN("Failed to get swapchain image for eye", frame_count);
    ...
    if (vkResult != VK_SUCCESS) {
            SPDLOG_WARN("Failed to begin command buffer for eye", frame_count);
    ...
    if (result != VK_SUCCESS) {
            SPDLOG_WARN("Failed to submit Vulkan copy command for eye", frame_count);
    ...
    if (frame_count < 5) {
            SPDLOG_WARN("DEBUG vr_copy: Copy completed successfully for eye", frame_count);
