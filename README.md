...
OpenXR support is currently implemented and in a working but work-in-progress state. This integration aims to enable XR functionality on supported Android devices (Quest 2/3)

Current status:

Basic OpenXR initialization and runtime interaction are functional.

Rendering pipeline support is present but still being actively refined.

More details and configuration options will be added as development continues.
